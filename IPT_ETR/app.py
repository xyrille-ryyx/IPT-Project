...
from models.data_processor import DataProcessor, TeenMentalHealthAnalyzer
...
analyzer = TeenMentalHealthAnalyzer(df)
...
def dashboard():
    ...
    if (selected_age_min and not selected_age_min.isdigit()) or (selected_age_max and not selected_age_max.isdigit()):
    ...
    summary = analyzer.summary(filtered_df)
    age_min_bound, age_max_bound = analyzer.age_bounds()
    chart_paths = Visualization(filtered_df).generate_dashboard_charts()
    ...
    if selected_gender or selected_age_min or selected_age_max:
        if summary['total_records'] == 0:
            ...
            if selected_age_min or selected_age_max:
                age_range_text = f"ages {selected_age_min or age_min_bound} to {selected_age_max or age_max_bound}"
                summary_parts.append(age_range_text)
            flash(f"Showing {summary['total_records']} records for {' - '.join(summary_parts)}.", 'success')
    ...
    return render_template(
        ...
        depression_rate=summary['depression_rate'],
        avg_social_media=summary['avg_social_media'],
